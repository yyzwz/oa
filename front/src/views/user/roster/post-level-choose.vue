  ...
  <div>
    <div style="display:flex;">
      ...
      <Poptip transfer trigger="click" placement="right" title="选择岗位" width="250">
        <Button icon="md-list" :disabled="disabled">选择岗位</Button>
        <div slot="content">
          <Input
            v-model="searchKey"
            suffix="ios-search"
...
export default {
  ...
  methods: {
    ...
    selectTree(v) {
      ...
      if (this.multiple) {
        this.postLevel.postLevelId = this.postLevelId;
      } else {
        this.postLevel.postLevelId = this.postLevelId[0];
      }
      this.postLevel.postLevelName = title;
      this.$emit("on-change", this.postLevel);
    },
    clearSelect() {
      this.postLevelId = [];
      this.postLevelTitle = "";
      this.initPostLevelData();
      this.postLevel = {postLevelId:'',postLevelName:''};
      this.$emit("on-change", this.postLevel);
      this.$emit("on-clear");
    },
    setData(ids, title) {
      this.postLevelTitle = title;
      if (this.multiple) {
        this.postLevel.postLevelId = ids;
      } else {
        this.postLevel.postLevelId = [];
        this.postLevel.postLevelId.push(ids);
      }
      this.postLevel.postLevelName = title;
      this.$emit("on-change", this.postLevel);
    }
