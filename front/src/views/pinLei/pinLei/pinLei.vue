...
<template>
  <div class="search">
    <Card>
      <Row class="operation">
        <Button  @click="add" type="primary" icon="md-add" v-show="showType=='tree'">添加子品类</Button>
        <Button  @click="addRoot" icon="md-add">添加一级品类</Button>
      ...
      <Row  type="flex" justify="start" v-show="showType=='tree'">
        ...
        <Col :md="15" :lg="13" :xl="9" style="margin-left:10px;">
          <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
            <FormItem label="上级品类" prop="parentTitle">
              <div style="display:flex;">
                <Input v-model="form.parentTitle" readonly style="margin-right:10px;" />
                <Poptip transfer trigger="click" placement="right-start" title="选择上级品类" width="250">
                  <Button icon="md-list">选择品类</Button>
            ...
            <FormItem label="品类名称" prop="title">
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="500">
      <Form ref="formAdd" :model="formAdd" :label-width="85" :rules="formValidate">
        <div v-if="showParent">
          <FormItem label="上级品类：">{{form.title}}</FormItem>
        </div>
        <FormItem label="品类名称" prop="title">
  ...
  methods: {
    ...
    submitEdit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          if (!this.form.id) {
            this.$Message.warning("请先点击选择要修改的品类");
            return;
          }
    ...
    add() {
      if (this.form.id == "" || this.form.id == null) {
        this.$Message.warning("请先点击选择一个品类");
        return;
      }
      this.modalTitle = "添加子品类";
