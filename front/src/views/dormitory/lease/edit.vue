    ...
    <Card>
      ...
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate" label-position="left" >        
        ...
        <FormItem label="入住时间" prop="checkTime">
          <DatePicker type="date" v-model="checkTimeTemp" @on-change="changeCheckTime" clearable style="width:570px"></DatePicker>
        </FormItem>
        <FormItem label="是否退租" prop="rentState"  >
          <i-switch v-model="form.rentState" true-color="#ff4949"></i-switch>
        </FormItem>
...
import { editDormitoryLease } from "./api.js";
...
export default {
  ...
  methods: {
    init() {
      this.handleReset();
      this.form = this.data;
      this.checkTimeTemp = this.data.checkTime;
    },
    changeCheckTime(){
      var b = new Date(this.checkTimeTemp);
    ...
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if(this.form.rentState == true){
            this.form.rentState = '已退租';
          }else{
            this.form.rentState = '在租';
          }
          editDormitoryLease(this.form).then(res => {
