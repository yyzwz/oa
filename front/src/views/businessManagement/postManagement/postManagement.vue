...
<template>
  <div class="search">
    <Card>
      <Row class="operation">
        <Button @click="add" type="primary" icon="md-add" v-show="showType=='tree'">添加子岗位</Button>
        <Button @click="addRoot" icon="md-add">添加一级岗位</Button>
...
export default {
  ...
  methods: {
    ...
    getParentList() {
      this.loading = true;
      getAllProfessionInTree().then(res => {
        this.loading = false;
        if (res.success) {
    ...
    loadData(item, callback) {
      loadProfession(item.id).then(res => {
        if (res.success) {
          res.result.forEach(function(e) {
            if (e.isParent) {
              e.loading = false;
    ...
    search() {
      if (this.searchKey) {
        this.loading = true;
        searchProfession({ title: this.searchKey }).then(res => {
          this.loading = false;
          if (res.success) {
            this.data = res.result;
          }
    ...
    submitEdit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          editProfession(this.form).then(res => {
    ...
    submitAdd() {
      this.$refs.formAdd.validate(valid => {
        if (valid) {
          this.submitLoading = true;
          addProfession(this.formAdd).then(res => {
            this.submitLoading = false;
            if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          deleteProfession({ids: ids}).then(res => {
            this.$Modal.remove();
            if (res.success) {
