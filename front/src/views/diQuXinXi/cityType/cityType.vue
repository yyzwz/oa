...
import { getCityTypeList, deleteCityType } from "./api.js";
// 根据你的实际添加编辑组件位置路径修改
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
        getCityTypeList(this.searchForm).then(res => {
          this.loading = false;
          if (res.success) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
            deleteCityType({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
            deleteCityType({ids: ids}).then(res => {
              this.$Modal.remove();
              if (res.success) {
                this.$Message.success("操作成功");
                this.clearSelectAll();
