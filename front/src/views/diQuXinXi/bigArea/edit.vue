  ...
  <div>
    <Card>
      ...
      <Form ref="form" :model="form" :label-width="100" :rules="formValidate" label-position="left" >        <FormItem label="编码" prop="number"  >
        ...
        <FormItem label="大区" prop="title"  >
          <Input v-model="form.title" clearable style="width:570px"/>
        </FormItem>
        <FormItem label="分公司" prop="sonCompany"  >
          <Select v-model="form.sonCompany" placeholder="请选择" clearable style="width: 200px">
              <Option
        ...
        <FormItem label="钉钉ID" prop="dingId"  >
          <Input v-model="form.dingId" clearable style="width:570px"/>
        </FormItem>
...
import { editBigArea,getSonCompany } from "./api.js";
export default {
  name: "edit",
  components: {
    },
  ...
  data() {
    ...
    formValidate: {
      ...
      // sonCompany: [{ required: true, message: "不能为空", trigger: "blur" }],
    },
      sonCompanyList:[],
    };
  },
  methods: {
    ...
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          editBigArea(this.form).then(res => {
