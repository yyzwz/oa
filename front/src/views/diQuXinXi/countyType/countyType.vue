    ...
    <Card v-show="currView=='index'">
        <Row @keydown.enter.native="handleSearch">
          <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
            ...
            <Form-item label="县级类型" prop="title">
              <Input type="text" v-model="searchForm.title" placeholder="请输入县级类型" clearable style="width: 200px"/>
            </Form-item>
            <span v-if="drop">
...
import { getCountyTypeList, deleteCountyType } from "./api.js";
// 根据你的实际添加编辑组件位置路径修改
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
        getCountyTypeList(this.searchForm).then(res => {
          this.loading = false;
          if (res.success) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
            deleteCountyType({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
            deleteCountyType({ids: ids}).then(res => {
              this.$Modal.remove();
              if (res.success) {
                this.$Message.success("操作成功");
                this.clearSelectAll();
