    ...
    <Card v-show="currView=='index'">
        <Row @keydown.enter.native="handleSearch">
          <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
            ...
            <Form-item label="城市类型" prop="title">
              ...
              <Input type="text" v-model="searchForm.title" placeholder="请输入城市类型" clearable style="width: 200px"/>
            </Form-item>
            <span v-if="drop">
            <Form-item label="物料编码" prop="title">
              <Input type="text" v-model="searchForm.wuLiao" placeholder="请输入物料编码" clearable style="width: 200px"/>
            </Form-item>
...
import { getCityLevelList, deleteCityLevel,getAllCityType } from "./api.js";
// 根据你的实际添加编辑组件位置路径修改
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
        getCityLevelList(this.searchForm).then(res => {
          this.loading = false;
          if (res.success) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
            deleteCityLevel({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
            deleteCityLevel({ids: ids}).then(res => {
              this.$Modal.remove();
              if (res.success) {
                this.$Message.success("操作成功");
                this.clearSelectAll();
