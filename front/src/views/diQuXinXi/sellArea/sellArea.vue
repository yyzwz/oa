    ...
    <Card v-show="currView=='index'">
        <Row @keydown.enter.native="handleSearch">
          <Form ref="searchForm" :model="searchForm" inline :label-width="70" class="search-form">
            ...
            <Form-item label="销售范围" prop="title">
                <Input type="text" v-model="searchForm.sellThing" placeholder="请输入销售范围" clearable style="width: 200px"/>
            </Form-item>
              <!-- <Input type="text" v-model="searchForm.area" placeholder="请输入区域" clearable style="width: 200px"/> -->
            
            <span v-if="drop">
              <Form-item label="区域" prop="city">
                <Select :placeholder="searchForm.area" clearable style="width: 200px" @on-change="changeArea">
                  <Option
...
import { getSellAreaList, deleteSellArea,getAllArea } from "./api.js";
// 根据你的实际添加编辑组件位置路径修改
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
        getSellAreaList(this.searchForm).then(res => {
          this.loading = false;
          if (res.success) {
    ...
    remove(v) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          // 删除
            deleteSellArea({ids: v.id}).then(res => {
              this.$Modal.remove();
              if (res.success) {
    ...
    delAll() {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          // 批量删除
            deleteSellArea({ids: ids}).then(res => {
              this.$Modal.remove();
              if (res.success) {
                this.$Message.success("操作成功");
                this.clearSelectAll();
